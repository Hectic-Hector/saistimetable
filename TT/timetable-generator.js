class TimetableGenerator {
    ...
    _initialize() {
        ...
        // Defensive: flatten teachers, skip undefined
        const teacherSet = new Set();
        Object.values(this.schoolData.teachers).forEach(subjectsObj => {
            Object.values(subjectsObj || {}).forEach(teacher => {
        ...
        // Defensive: skip undefined subjects/teachers
        allClasses.forEach(className => {
    ...
    _getAvailableLessonSlots(className, day) {
        const division = this._getClassDivision(className);
        const ds = this.schoolData.divisionSchedules[division];
        if (!ds) return []; // Defensive
        let slots = [...ds.lessonSlots];
    ...
    _countSubjectOnDay(className, subject, day) {
        if (!this.timetables[className] || !this.timetables[className][day]) return 0;
    ...
    canAssignLesson(className, subject, teacher, day, slot) {
        ...
        if (!teacher || !this.teacherStats[teacher]) return { valid: false, reason: "Teacher not found in stats" };
    ...
    _scheduleLesson(lesson, allowedDays) {
        let { periods: periodsToSchedule, className, subject, teacher } = lesson;
        const division = this._getClassDivision(className);
    ...
    _scheduleSpecificPeriods({ className, subject, teacher }, allowedDays, numPeriods) {
        if (!teacher) return false; // Defensive
